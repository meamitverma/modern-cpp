...
public:
    ...
    PaymentCheck calculate(int amount2)
    {
        this->amount2 = amount2;
        return *this;
    }
    PaymentCheck paymentby_method(string fileName)
    {
        this->filename = fileName;
        // cout << "Payment method "<< fileName <<  endl;
        return *this;
    }
    PaymentCheck useLib(int libName)
    {
        this->libName = libName;
        // cout << "Payed through "<< libName <<  endl;
        return *this;
    } // Card , Gpay Cash
    PaymentCheck store(string path)
    {
        this->path = path;
    ...
    void payment_bill_download()
    {
        if (isConnect == true)
        {
            ...
            case GPAY:
                pv.payment_bill_download(url, amount2, filename, path);
            default:
                cout << "wrong lib name" << endl;
            }
        }
        else
        {
...
{
public:
    Bill &product(const std::string &productName)
    {
        product_ = productName;
        return *this;
    }

    Bill &brand(const std::string &brandName)
    {
        brand_ = brandName;
        return *this;
    }

    Bill &cost(double costAmount)
    {
        cost_ = costAmount;
        return *this;
    }

    Bill &tax(double taxPercentage)
    {
        tax_ = taxPercentage;
        return *this;
    }
